...
def get_app_listvariants(app_build_path: Path) -> tuple[str, list[str]]:
    ...
    if not variants:
        raise ValueError("At least one variant should be defined in the app Makefile")
    ...
    for line in ret.split("\n"):
        ...
        if "APP_LOAD_PARAMS=" in line:
            ...
            if key in ["curve", "path", "tlvraw", "dep"]:
                ...
                listparams[key] = value.strip().split(" ")
            else:
...
def find_app_elf(app_build_path: Path, target: str | None) -> Path | None:
...
def get_embedded_glyphs(elf: Path) -> set[str] | None:
...
def get_accepted_geometries_for_target(target: str) -> set[str]:
    ...
    if target_lower == "stax":
        return {"32x32", "64x64"}
    # Unknown target
    raise ValueError(f"No supported glyph sizes defined for target '{target}'")
...
def get_glyph_dimensions(glyph_path: str, app_build_path: Path) -> str:
    ...
    if identify is None:
        raise RuntimeError("'identify' (ImageMagick) is not installed or not on PATH")
    ...
    try:
        ...
        raise RuntimeError(f"'identify' timed out reading {glyph_path}") from None
    except (ValueError, FileNotFoundError) as err:
        raise RuntimeError(f"Failed to get dimensions for {glyph_path}: {err}") from err
...
def save_app_params(app_build_path: Path, json_path: Path) -> None:
    ...
    if embedded is not None:
        if not isinstance(target, str):
            raise TypeError(f"Expected str for target, got {type(target)}")
        ...
        for _, params in ret["VARIANTS"].items():
...
def is_allowed_makefile(app_build_path: Path) -> bool:
    ...
    with open(makefile_path) as f:
        for line in f:
            ...
            if any(
                allowed_makefile in stripped_line and not stripped_line.startswith("#") for allowed_makefile in allowed_makefiles
            ):
                return True
