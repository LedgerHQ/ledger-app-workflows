...
ALL_CHECKS="manifest icons app_load_params makefile readme clang"
...
if [[ (-z ${REQUESTED_CHECK}) || ("${REQUESTED_CHECK}" =~ (manifest|clang)) ]]; then
...
if [[ (-z ${TARGET}) && ("${REQUESTED_CHECK}" =~ (manifest|clang)) ]]; then
