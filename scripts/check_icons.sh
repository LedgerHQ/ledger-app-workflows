...
get_icon_from_makefile () {
	cat Makefile | grep "ICONNAME" | grep "$1" | cut -f2 -d'=' | sed 's/^[ \t]*//;s/[ \t]*$//'
}
nanos_icon_file=$(get_icon_from_makefile nanos)
nanox_icon_file=$(get_icon_from_makefile nanox)

check_icon "$nanos_icon_file" "16x16"
check_icon "$nanox_icon_file" "14x14"

find ./glyphs/ -type f -print0 | while IFS= read -r -d '' file; do check_icon "$file" "no-check"; done
